...
- **A fast enough camera.** Two different limits apply:
  - **Minimum: 2× the display's refresh rate.** A slower camera can miss a frame that is shown for a single refresh, once the
    panel's transition and the exposure are taken into account. Calibration warns below it. The simulated camera still finds every
    frame at 1.7×, but its panel is idealised, so keep the margin.
  - **Precision: 4× the frame rate you measure, 8× is better.** Display times are off by up to about one camera frame period (see
    the measurements below). At 4× that is up to a quarter of a frame time, at 8× an eighth.

  **For the usual target, 60 fps on a 60 Hz display, 240 fps is enough and 500 fps is better.** A phone's 240 fps slow motion or
  a 240–330 fps UVC camera works.

  | Frame rate (on a display at the same rate) | Frame time | Minimum (2× refresh) | Recommended (4×) | Better (8×) |
  | ------------------------------------------ | ---------- | -------------------- | ---------------- | ----------- |
  | 30 fps (on 60 Hz)                          | 33.3 ms    | 120 fps              | 120 fps          | 240 fps     |
  | **60 fps**                                 | 16.7 ms    | 120 fps              | **240 fps**      | **500 fps** |
  | 120 fps                                    | 8.3 ms     | 240 fps              | 500 fps          | 1000 fps    |
  | 144 fps                                    | 6.9 ms     | 300 fps              | 600 fps          | 1200 fps    |
  | 240 fps                                    | 4.2 ms     | 500 fps              | 1000 fps         | 2000 fps    |
  | 500 fps                                    | 2.0 ms     | 1000 fps             | 2000 fps         | 4000 fps    |

  A game below the refresh rate still needs 2× the display's rate: 30 fps on a 144 Hz display needs about 300 fps. Tear detection
  between the two zones needs more: the zones must be at least 4 camera frames apart in the scanout, which is about 300 fps on a
  60 Hz display and about 700 fps at 144 Hz.

  Measured on the **simulated** camera filming a 60 Hz display (not real hardware): the error of each measured display time
  against the simulation, and in brackets as a share of the 16.7 ms frame time. Regenerate it with
  `python tools/camera_rate_table.py --update-doc`; the simulation has a fixed seed, so the numbers repeat.

  <!-- camera-rate-table: generated by tools/camera_rate_table.py -->
...
GUI share it. Replacing or deleting a saved camera keeps the old file in `camera-rigs/backup` (see
[Configuration](../README.md#configuration)), so a mistake can be undone.
